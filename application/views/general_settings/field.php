        ...
        <div class="content-header row">
            <div class="content-header-left col-md-9 col-12 mb-2">
                <div class="row breadcrumbs-top">
                    <div class="col-12">
                        <h2 class="content-header-title float-left mb-0">Field</h2>
                        <div class="breadcrumb-wrapper col-12">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item active">Field
                                </li>
                            </ol>
                        </div>
                    </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header bg-primary white">
                    <h4 class="modal-title white" id="myModalLabel_add">Add Field</h4>
                </div>
                <div class="modal-body">
                    <div class="form-group">
                        <label for="FieldName">Field: </label>
                        <input type="text" class="form-control FieldName" id="FieldName">
                    </div>
                </div>
...
<?php if (isset($permission[0]->CanEdit) && $permission[0]->CanEdit == 1) { ?>
    <div class="modal fade text-left" id="editModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel_edit"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header bg-primary white">
                    <h4 class="modal-title white" id="myModalLabel_edit">Edit Field</h4>
                    <input type="hidden" id="edit_idField" name="edit_idField">
                </div>
                <div class="modal-body">
                    <div class="form-group">
                        <label for="edit_FieldName">Field: </label>
                        <input type="text" class="form-control edit_FieldName" id="edit_FieldName">
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn grey btn-secondary" data-dismiss="modal">Close</button>
...
<?php if (isset($permission[0]->CanDelete) && $permission[0]->CanDelete == 1) { ?>
    <div class="modal fade text-left" id="deleteModal" tabindex="-1" role="dialog"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header bg-primary white">
                    <h4 class="modal-title white" id="myModalLabel_delete">Delete Field</h4>
                    <input type="hidden" id="delete_idField" name="delete_idField">
                </div>
                <div class="modal-body">
...
<script>
    ...
    function addData() {
        $('#FieldName').css('border', '1px solid #babfc7');
        var data = {};
        data['FieldName'] = $('#FieldName').val();
        if (data['FieldName'] == '' || data['FieldName'] == undefined) {
            $('#FieldName').css('border', '1px solid red');
            toastMsg('Field', 'Invalid Field Name', 'error');
        } else {
            showloader();
            $('.mybtn').attr('disabled', 'disabled');
            CallAjax('<?php echo base_url('index.php/general_settings/Field/addFieldData'); ?>', data, 'POST', function (result) {
                hideloader();
                if (result == 1) {
                    ...
                    toastMsg('Field', 'Field Name already exist', 'error');
                } else {
    ...
    function getEdit(obj) {
        ...
        if (data['id'] != '' && data['id'] != undefined) {
            CallAjax('<?php echo base_url('index.php/general_settings/Field/getFieldEdit')?>', data, 'POST', function (result) {
                if (result != '' && JSON.parse(result).length > 0) {
                    var a = JSON.parse(result);
                    try {
                        $('#edit_idField').val(data['id']);
                        $('#edit_FieldName').val(a[0]['field']);
                    } catch (e) {
                    }
                    $('#editModal').modal('show');
                } else {
                    toastMsg('Field', 'Invalid Field', 'error');
                }
            });
        }
    }
    ...
    function editData() {
        $('#edit_FieldName').css('border', '1px solid #babfc7');
        var flag = 0;
        var data = {};
        data['idField'] = $('#edit_idField').val();
        data['FieldName'] = $('#edit_FieldName').val();
        if (data['idField'] == '' || data['idField'] == undefined || data['idField'].length < 1) {
            flag = 1;
            return false;
        }
        if (data['FieldName'] == '' || data['FieldName'] == undefined || data['FieldName'].length < 1) {
            $('#edit_FieldName').css('border', '1px solid red');
            toastMsg('Field', 'Invalid Field Name', 'error');
            flag = 1;
            return false;
        }
        if (flag === 0) {
            CallAjax('<?php echo base_url('index.php/general_settings/Field/editFieldData')?>', data, 'POST', function (res) {
                if (res == 1) {
                    $('#editModal').modal('hide');
                    toastMsg('Field', 'Successfully Edited', 'success');
                    setTimeout(function () {
                        window.location.reload();
                    }, 500);
                } else if (res == 2) {
                    toastMsg('Field', 'Something went wrong', 'error');
                } else if (res == 3) {
                    toastMsg('Field', 'Invalid Field', 'error');
                } else if (res == 4) {
                    toastMsg('Field', 'Field Name already exist', 'error');
    ...
    function getDelete(obj) {
        ...
        $('#delete_idField').val(id);
        $('#deleteModal').modal('show');
    }

    function deleteData() {
        var data = {};
        data['idField'] = $('#delete_idField').val();
        if (data['idField'] == '' || data['idField'] == undefined || data['idField'] == 0) {
            toastMsg('Field', 'Something went wrong', 'error');
            return false;
        } else {
            CallAjax('<?php echo base_url('index.php/general_settings/Field/deleteFieldData')?>', data, 'POST', function (res) {
                if (res == 1) {
                    $('#deleteModal').modal('hide');
                    toastMsg('Field', 'Successfully Deleted', 'success');
                    setTimeout(function () {
                        window.location.reload();
                    }, 500);
                } else if (res == 2) {
