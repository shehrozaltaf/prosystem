                                            ...
                                            if (isset($data) && $data != '') {
                                                foreach ($data as $v) {
                                                    ...
                                                    <tr>
                                                        <td><?php echo $SNo ?></td>
                                                        <td class="mycol"
                                                            data-txt="<?php echo $v->entity ?>"><?php echo $v->entity ?></td>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="addLocationModalLabel">Add Entity</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <label for="entity" class="label-control">Entity</label>
                <input type="text" class="form-control" id="entity" maxlength="120" name="entity"
                       style="text-transform: uppercase;"
                       onkeypress="return lettersOnly_WithSpace();"
                       autocomplete="entity" required>
            </div>
            <div class="modal-footer">
                <button id="cmdconfirm" type="button" class="btn grey btn-primary" onclick="addEntity();">Save
                </button>
                <button id="cmdUpdate" type="button" class="btn grey btn-primary" onclick="editEntity();">Update
                </button>
...
<script>
    ...
    $(document).on("click", "#cmdadd", function () {
        ...
        $("#addLocationModalLabel").text("Add Entity");
    });
    ...
    function getEdit(obj) {
        ...
        $("#entity").val(loc);
        $("#addLocationModal").modal('show');
        $("#addLocationModalLabel").text("Update Entity");
    ...
    function deleteData() {
        ...
        if (data['idActual'] == '' || data['idActual'] == undefined || data['idActual'] == 0) {
            ...
            CallAjax('<?php echo base_url('index.php/hr_controllers/EmpEntity/deleteData')?>', data, 'POST', function (res) {
                if (res == 1) {
    ...
    function addEntity() {
        let addData = {};
        addData["id"] = "0";
        addData["entity"] = $('#entity').val();

        if ($('#entity').val() == '' || $('#entity').val() == undefined || $('#entity').val() == 0 ||
            $('#entity').val() == null ||
            $('#entity').val() == 'NULL'
        ) {
            toastMsg('Group', 'Please enter entity', 'error');
            $('#entity').focus();
            return false;
        } else {

            CallAjax('<?php echo base_url('index.php/hr_controllers/EmpEntity/getEntityAlreadyExists'); ?>', addData, 'POST', function (result) {
                ...
                if (result != '' && result != null) {
                    ...
                    try {
                        if (a[0] != null) {
                            toastMsg('Error', 'Entity already exists', 'error');
                            $("#entity").focus();
                        } else {

                            CallAjax('<?php echo base_url('index.php/hr_controllers/EmpEntity/addEntity')?>', addData, 'POST', function (res) {
                                if (res == 1) {
                                    //$('#deleteModal').modal('hide');
                                    toastMsg('Group', 'Record Successfully', 'success');
                                    setTimeout(function () {
                                        //window.location.reload();
                                        window.location.href = '<?php echo base_url('index.php/hr_controllers/EmpEntity')?>';
                                    }, 500);
                                } else if (res == 2) {
                                    toastMsg('Group', 'Something went wrong', 'error');
                                } else if (res == 3) {
    ...
    function editEntity() {
        ...
        if (editData == '' || editData == undefined || editData == 0 ||
            ...
            CallAjax('<?php echo base_url('index.php/hr_controllers/EmpEntity/getEntityAlreadyExists'); ?>', editData, 'POST', function (result) {
                ...
                if (result != '' && result != null) {
                    ...
                    try {
                        if (a[0] != null) {
                            ...
                            CallAjax('<?php echo base_url('index.php/hr_controllers/EmpEntity/editEntity')?>', editData, 'POST', function (res) {
                                ...
                                if (res == 1) {
                                    ...
                                    toastMsg('Group', 'Invalid Entity', 'error');
                                }
