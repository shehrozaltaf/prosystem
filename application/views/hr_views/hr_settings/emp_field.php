                    ...
                    <div class="col-12">
                        ...
                        <div class="breadcrumb-wrapper col-12">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item active"><a href="javascript:void(0)">Field</a></li>
                            </ol>
                        </div>
                    </div>
                </div>
                                            ...
                                            if (isset($data) && $data != '') {
                                                foreach ($data as $v) {
                                                    ...
                                                    <tr>
                                                        <td><?php echo $SNo ?></td>
                                                        <td class="mycol"
                                                            data-txt="<?php echo $v->field ?>"><?php echo $v->field ?></td>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="addLocationModalLabel">Add Field</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <label for="location" class="label-control">Field</label>
                <input type="text" class="form-control" id="field" maxlength="80" name="field"
                       style="text-transform: capitalize;"
                       onkeypress="return lettersOnly_WithSpace();"
                       autocomplete="location" required>
            </div>
            <div class="modal-footer">
                <button id="cmdconfirm" type="button" class="btn grey btn-primary" onclick="addField();">Save
                </button>
                <button id="cmdUpdate" type="button" class="btn grey btn-primary" onclick="editField();">Update
                </button>
...
<script>
    ...
    $(document).on("click", "#cmdadd", function () {
        ...
        $("#addLocationModalLabel").text("Add Field");
    });
    ...
    function getEdit(obj) {
        ...
        $("#field").val(loc);
        $("#addLocationModal").modal('show');
        $("#addLocationModalLabel").text("Update Field");
    ...
    function deleteData() {
        ...
        if (data['idActual'] == '' || data['idActual'] == undefined || data['idActual'] == 0) {
            ...
            CallAjax('<?php echo base_url('index.php/hr_controllers/EmpField/deleteData')?>', data, 'POST', function (res) {
                if (res == 1) {
    ...
    function addField() {
        let addData = {};
        addData["id"] = "0";
        addData["field"] = $('#field').val();

        if ($('#field').val() == '' || $('#field').val() == undefined || $('#field').val() == 0 ||
            $('#field').val() == null ||
            $('#field').val() == 'NULL'
        ) {
            toastMsg('Group', 'Please enter field', 'error');
            $('#field').focus();
            return false;
        } else {

            CallAjax('<?php echo base_url('index.php/hr_controllers/EmpField/getFieldAlreadyExists'); ?>', addData, 'POST', function (result) {
                ...
                if (result != '' && result != null) {
                    ...
                    try {
                        if (a[0] != null) {
                            toastMsg('Error', 'Field already exists', 'error');
                            $("#field").focus();
                        } else {

                            CallAjax('<?php echo base_url('index.php/hr_controllers/EmpField/addField')?>', addData, 'POST', function (res) {
                                if (res == 1) {
                                    //$('#deleteModal').modal('hide');
                                    toastMsg('Group', 'Record Successfully', 'success');
                                    setTimeout(function () {
                                        //window.location.reload();
                                        window.location.href = '<?php echo base_url('index.php/hr_controllers/EmpField')?>';
                                    }, 500);
                                } else if (res == 2) {
                                    toastMsg('Group', 'Something went wrong', 'error');
                                } else if (res == 3) {
                                    toastMsg('Group', 'Invalid Field', 'error');
                                }
    ...
    function editField() {
        ...
        if (editData == '' || editData == undefined || editData == 0 ||
            ...
            CallAjax('<?php echo base_url('index.php/hr_controllers/EmpField/getFieldAlreadyExists'); ?>', editData, 'POST', function (result) {
                ...
                if (result != '' && result != null) {
                    ...
                    try {
                        if (a[0] != null) {
                            ...
                            CallAjax('<?php echo base_url('index.php/hr_controllers/EmpField/editField')?>', editData, 'POST', function (res) {
