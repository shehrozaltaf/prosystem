        ...
        <div class="content-header row">
            <div class="content-header-left col-md-9 col-12 mb-2">
                <div class="row breadcrumbs-top">
                    <div class="col-12">
                        <h2 class="content-header-title float-left mb-0">Employee Type</h2>
                        <div class="breadcrumb-wrapper col-12">
                            <ol class="breadcrumb">
                                <li class="breadcrumb-item">
                                    <a href="<?php base_url() ?>">Home</a>
                                </li>
                                <li class="breadcrumb-item active"><a href="javascript:void(0)">Employee Type</a></li>
                            </ol>
                        </div>
                    </div>
                </div>
                                            ...
                                            if (isset($data) && $data != '') {
                                                foreach ($data as $v) {
                                                    ...
                                                    <tr>
                                                        <td><?php echo $SNo ?></td>
                                                        <td class="mycol"
                                                            data-txt="<?php echo $v->emptype ?>"><?php echo $v->emptype ?></td>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="addLocationModalLabel">Add Employment Type</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <label for="location" class="label-control">Employment Type</label>
                <input type="text" class="form-control" id="emptype" maxlength="80" name="emptype"
                       style="text-transform: capitalize;"
                       onkeypress="return lettersOnly_WithSpace();"
                       autocomplete="location" required>
            </div>
            <div class="modal-footer">
                <button id="cmdconfirm" type="button" class="btn grey btn-primary" onclick="addEmpType();">Save
                </button>
                <button id="cmdUpdate" type="button" class="btn grey btn-primary" onclick="editEmpType();">Update
                </button>
...
<script>
    ...
    $(document).on("click", "#cmdadd", function () {
        ...
        $("#addLocationModalLabel").text("Add Employment Type");
    });
    ...
    function getEdit(obj) {
        ...
        $("#emptype").val(loc);
        $("#addLocationModal").modal('show');
        $("#addLocationModalLabel").text("Update Employment Type");
    ...
    function deleteData() {
        ...
        if (data['idActual'] == '' || data['idActual'] == undefined || data['idActual'] == 0) {
            ...
            CallAjax('<?php echo base_url('index.php/hr_controllers/EmpType/deleteData')?>', data, 'POST', function (res) {
                if (res == 1) {
    ...
    function addEmpType() {
        let addData = {};
        addData["id"] = "0";
        addData["emptype"] = $('#emptype').val();

        if ($('#emptype').val() == '' || $('#emptype').val() == undefined || $('#emptype').val() == 0 ||
            $('#emptype').val() == null ||
            $('#emptype').val() == 'NULL'
        ) {
            toastMsg('Group', 'Please enter employment type', 'error');
            $('#emptype').focus();
            return false;
        } else {

            CallAjax('<?php echo base_url('index.php/hr_controllers/EmpType/getEmpTypeAlreadyExists'); ?>', addData, 'POST', function (result) {
                ...
                if (result != '' && result != null) {
                    ...
                    try {
                        if (a[0] != null) {
                            toastMsg('Error', 'Employment Type already exists', 'error');
                            $("#emptype").focus();
                        } else {

                            CallAjax('<?php echo base_url('index.php/hr_controllers/EmpType/addEmpType')?>', addData, 'POST', function (res) {
                                if (res == 1) {
                                    //$('#deleteModal').modal('hide');
                                    toastMsg('Group', 'Record Successfully', 'success');
                                    setTimeout(function () {
                                        //window.location.reload();
                                        window.location.href = '<?php echo base_url('index.php/hr_controllers/EmpType')?>';
                                    }, 500);
                                } else if (res == 2) {
                                    toastMsg('Group', 'Something went wrong', 'error');
                                } else if (res == 3) {
                                    toastMsg('Group', 'Invalid Employment Type', 'error');
                                }
    ...
    function editEmpType() {
        ...
        if ($('#emptype').val() == '' || $('#emptype').val() == undefined || $('#emptype').val() == 0 ||
            ...
            CallAjax('<?php echo base_url('index.php/hr_controllers/EmpType/getEmpTypeAlreadyExists'); ?>', editData, 'POST', function (result) {
                ...
                if (result != '' && result != null) {
                    ...
                    try {
                        if (a[0] != null) {
                            ...
                            CallAjax('<?php echo base_url('index.php/hr_controllers/EmpType/editEmpType')?>', editData, 'POST', function (res) {
